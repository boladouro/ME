...
# a = 0, b = 100 e c = 50
set.seed(2023)
ggplot(tibble(x = rtriang(10000, 0, 100, 50)), aes(x)) + 
  geom_histogram(aes(y = after_stat(density)), bins = 20, colour = "black", fill = "white") +
...
aceitacao_rejeicao_points <- function(n, a, b, c) {
  C <- dtriang(c, a, b, c)
  f <- function(x) dtriang(x, a, b, c)
  points <- tibble(x = numeric(0), y = numeric(0), accepted = logical(0))
  for(i in 1:n) {
    x <- runif(1, a, b)
    u <- runif(1, 0, 1)
    if(u <= f(x)/C**) {
      points <- points %>% add_row(x = x, y = u, accepted = TRUE)
    } else {
      points <- points %>% add_row(x = x, y = u, accepted = FALSE)
    }
  }
  return(points)
}
df <- aceitacao_rejeicao_points(10000, 1, 100, 50)
ggplot(df, aes(x, y)) + 
  geom_point(aes(color = accepted), alpha = 0.5) +
  labs(title = "Aceitação e rejeição", x = "x", y = "y") +
  scale_colour_manual("", values = c("TRUE" = "#F73859", "FALSE" = "#404B69"), guide = guide_legend())
```
...
#   X <- \() runif(1, a, b)
#   # c: max of f, so that c >= f(x) / g(x) for all x
#   C <- f(c) ## o maximo da funcao triangular e no dtriang(c)
#   # monte carlo (u(0, 1) <= f(x) / (c*g(x)) <=> u(0, c*g(x)) <= f(x))
#   u <- \(x) runif(n=1, min=0, max=1)
#   
#   ret <- c()
